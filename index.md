---
layout: default
title: Home
permalink: /
---
<section class="hero stagger">
  <img class="avatar" src="{{ '/assets/profile-placeholder.svg' | relative_url }}" alt="Profile placeholder image">
  <div>
    <p class="eyebrow">Princeton University · M.S.E. Computer Science (2025-2027)</p>
    <h1>Daniel Lima Braga</h1>
    <p>
      I am a master's student at Princeton focused on machine learning, scientific discovery, and computational cognitive science.
      I currently do research in the
      <a href="https://cocosci.princeton.edu/" target="_blank" rel="noopener">Computational Cognitive Science Lab</a>
      advised by Prof. Tom Griffiths.
    </p>
    <div class="cta-row">
      <a class="btn primary" href="mailto:dbraga@princeton.edu">Contact Me</a>
      <a class="btn" href="{{ '/assets/Resume_Spring_2026_latest.pdf' | relative_url }}" target="_blank" rel="noopener">Download CV</a>
      <a class="btn" href="https://github.com/bragadaniel14" target="_blank" rel="noopener">GitHub</a>
    </div>
  </div>
</section>

<section class="section-block stagger">
  <h2>Career Snapshot</h2>
  <div class="card-grid">
    <article class="card">
      <h3>Current Focus</h3>
      <p>Automated scientific discovery with large language models and latent structure inference from behavioral data.</p>
    </article>
    <article class="card">
      <h3>Industry Background</h3>
      <p>Senior Software Engineer at Bloomberg (2016-2025), with leadership and large-scale data infrastructure experience.</p>
    </article>
    <article class="card">
      <h3>Core Stack</h3>
      <p>Python, Java, JavaScript, PyTorch, JAX, Spark, AWS S3, PostgreSQL, Docker, Git.</p>
    </article>
  </div>
</section>

<section class="section-block stagger">
  <h2>Experience</h2>
  <div class="timeline">
    <article class="timeline-item">
      <p class="meta">September 2025 - Present</p>
      <h3>Graduate Researcher · Princeton University</h3>
      <p>Computational Cognitive Science Lab. Building evaluation frameworks for LLM-based representation discovery.</p>
    </article>
    <article class="timeline-item">
      <p class="meta">September 2016 - July 2025</p>
      <h3>Senior Software Engineer · Bloomberg L.P.</h3>
      <p>Built distributed financial-document data pipelines (Kafka, Spark, AWS), led security remediation, and mentored engineers.</p>
    </article>
    <article class="timeline-item">
      <p class="meta">2018 - 2019</p>
      <h3>Undergraduate Researcher · Princeton University</h3>
      <p>Projects in computational biology, computer vision, and theoretical algorithms.</p>
    </article>
  </div>
</section>

<section class="section-block stagger">
  <h2>Teaching and Classes</h2>
  <div class="card-grid two-col">
    <article class="card">
      <h3>Teaching / Mentoring</h3>
      <ul>
        <li>Princeton University Recruiting Captain at Bloomberg (workshops and interviews).</li>
        <li>TA courses at Princeton: details coming soon.</li>
      </ul>
    </article>
    <article class="card">
      <h3>Selected Coursework</h3>
      <ul>
        <li>Foundations of Reinforcement Learning</li>
        <li>Gen AI Models of Cellular Dynamics</li>
        <li>Computational Models of Cognition</li>
        <li>Graph Neural Nets (in progress)</li>
        <li>Theoretical Machine Learning (in progress)</li>
      </ul>
    </article>
  </div>
</section>
